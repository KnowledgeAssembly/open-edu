...
Progress is saved to IndexedDB via `@open-edu/storage` with 6 typed object stores: `courses`, `progress`, `badges`, `cards`, `search-indexes`, and `preferences`. On return visits, the app resumes from the last uncompleted node. All persistence was migrated from `localStorage` to IndexedDB for quota-aware, structured storage.

## Progressive Web App

The learner app is a full PWA with offline-first support. See [PWA](pwa.md) for the complete architecture.

### Install Prompt

When the app is installable, an `InstallPrompt` banner appears on the catalog page. The `useInstallPrompt` hook bridges `@open-edu/pwa-core` install detection to React. Clicking "Install" triggers the browser's native install prompt.

### Update Detection

The `useUpdatePrompt` hook monitors for new service worker versions. When an update is available, an `UpdatePrompt` notification appears in the app shell. Updates never force-reload or interrupt learning — the user chooses when to apply.

### Offline Mode

The `OfflineBanner` component displays a persistent banner when the device goes offline, indicating that cached content is available. The `useOnlineStatus` hook wraps `@open-edu/pwa-core` connectivity detection.

### Storage Management

The `StorageSettingsPage` shows:

- Storage usage (via `useStorageUsage` hook wrapping `getStorageUsage`)
- List of downloaded courses with delete buttons (`DownloadedCourseList`)
- Storage usage card with quota visualization
...
- **NetworkFirst** for API/metadata endpoints
- **StaleWhileRevalidate** for catalog listings
...
Card progress is persisted to IndexedDB via `@open-edu/storage` in the `cards` object store.

| Store   | Key      | Data                             |
| ------- | -------- | -------------------------------- |
| `cards` | `cardId` | `{ level, unlockedAt }` per card |
