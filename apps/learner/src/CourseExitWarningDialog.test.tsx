...
describe('CourseExitWarningDialog', () => {
  it('renders when open', async () => {
    renderWithProvider(<CourseExitWarningDialog open onStay={vi.fn()} onLeave={vi.fn()} />);
  ...
  it('does not render when closed', () => {
    renderWithProvider(<CourseExitWarningDialog open={false} onStay={vi.fn()} onLeave={vi.fn()} />);
    expect(screen.queryByTestId('exit-warning-dialog')).not.toBeInTheDocument();
  });

  it('calls onStay when Stay button is clicked', async () => {
    const onStay = vi.fn();
    renderWithProvider(<CourseExitWarningDialog open onStay={onStay} onLeave={vi.fn()} />);
  ...
  it('calls onLeave when Leave button is clicked', async () => {
    const onLeave = vi.fn();
    renderWithProvider(<CourseExitWarningDialog open onStay={vi.fn()} onLeave={onLeave} />);
