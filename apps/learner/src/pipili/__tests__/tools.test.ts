...
describe('createToolRegistry', () => {
  ...
  it('getCurrentPageContext executes without error', async () => {
    const tools = createToolRegistry(contextGetter);
    const result = await tools.getCurrentPageContext.execute({}, {} as never);
  ...
  it('searchNotes validates query parameter', async () => {
    const tools = createToolRegistry(contextGetter);
    const result = await tools.searchNotes.execute({ query: 'math', maxResults: 3 }, {} as never);
    expect(result.query).toBe('math');
  });

  it('getRelevantNotes validates noteIds array', async () => {
    const tools = createToolRegistry(contextGetter);
    const result = await tools.getRelevantNotes.execute({ noteIds: ['n1', 'n2'] }, {} as never);
  ...
  it('all tools have explicit Zod schemas', () => {
    const tools = createToolRegistry(contextGetter);
