...
describe('BadgeToast', () => {
  it('renders badge name when visible', () => {
    renderWithProvider(<BadgeToast badgeName="Test Badge" visible={true} />);
  ...
  it('does not render when not visible', () => {
    renderWithProvider(<BadgeToast badgeName="Test Badge" visible={false} />);
  ...
  it('calls onDismiss after auto-dismiss timeout', async () => {
    ...
    renderWithProvider(<BadgeToast badgeName="Test" visible={true} onDismiss={onDismiss} />);
    act(() => {
  ...
  it('renders with data-testid badge-toast', () => {
    renderWithProvider(<BadgeToast badgeName="Test" visible={true} />);
    expect(screen.getByTestId('badge-toast')).toBeInTheDocument();
  });

  it('unmounts from DOM after dismiss with animation delay', () => {
    vi.useFakeTimers({ toFake: ['setTimeout', 'requestAnimationFrame'] });
    const { rerender } = renderWithProvider(<BadgeToast badgeName="Test" visible={true} />);
  ...
  it('sets aria-label containing badge name', () => {
    renderWithProvider(<BadgeToast badgeName="Super Star" visible={true} />);
    const toast = screen.getByTestId('badge-toast');
