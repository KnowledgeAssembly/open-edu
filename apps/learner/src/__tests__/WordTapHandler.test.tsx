...
function renderWithWordTap(children: React.ReactNode) {
  ...
  const spyCaret = vi.spyOn(document, 'caretRangeFromPoint').mockReturnValue({
    startContainer: wordEl.firstChild!,
    startOffset: 0,
    endContainer: wordEl.firstChild!,
    endOffset: wordEl.textContent!.length,
    collapsed: true,
  } as unknown as Range);
...
function clickWord(wordEl: HTMLElement, x = 50, y = 50) {
  ...
  fireEvent.click(wordEl, { clientX: x, clientY: y });
}
