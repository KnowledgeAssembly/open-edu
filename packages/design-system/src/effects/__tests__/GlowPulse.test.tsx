...
describe('GlowPulse', () => {
  ...
  it('applies className prop', () => {
    render(
      <GlowPulse className="custom-class">
        <span>child</span>
      </GlowPulse>,
    );
    const container = screen.getByTestId('glow-pulse');
  ...
  it('applies duration prop to inline style', () => {
    render(
    ...
    const container = screen.getByTestId('glow-pulse');
    expect(container.getAttribute('style')).toContain('glow-pulse 2.5s');
  });

  it('uses default duration of 1.2s', () => {
    render(
      <GlowPulse>
        <span>child</span>
      </GlowPulse>,
    ...
    const container = screen.getByTestId('glow-pulse');
    expect(container.getAttribute('style')).toContain('glow-pulse 1.2s');
  });

  it('renders with custom color prop', () => {
    const { container } = render(
      <GlowPulse color="#ff0000">
        <span>child</span>
      </GlowPulse>,
    );
    const styleTag = container.querySelector('style');
