  ...
  ({ variant = 'lockup', size = 'md', className, ...props }, ref) => {
    ...
    if (variant === 'wordmark') {
      return (
        <div
          ref={ref}
          className={cn('inline-flex items-center gap-0.5 font-light tracking-tight', s.text, className)}
          {...props}
        >
          <span className="text-foreground">open</span>
          <svg width={s.symbol * 0.3} height={s.symbol * 0.3} viewBox="0 0 20 20" className="text-primary fill-current self-start mt-0.5">
    ...
    return (
      <div ref={ref} className={cn('inline-flex items-center gap-2', className)} {...props}>
        ...
        <div className="flex flex-col">
          <span className={cn('font-semibold leading-tight text-foreground', s.text)}>
            OpenEdu
          </span>
