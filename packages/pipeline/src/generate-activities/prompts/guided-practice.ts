...
## Available Widgets
You may output \`type: "exercise"\` for text problems or \`type: "widget"\` with \`interactive: true\` for visual interactive practice. Choose \`widget\` when the concept lends itself to matching, sorting, sequencing, filling blanks, labeling, or drag-drop activities with hints.
...
| \`core.hotspot\` | Click-to-identify parts of an image | \`image\`, \`altText\`, \`hotspots[{id,x,y,label}]\`, \`hints\` |
| \`core.timeline\` | Ordering historical events | \`events[{id,title,date}]\`, \`layout\`, \`hints\` |
...
| \`math.number-line\` | Number line, integer placement | \`min\`, \`max\`, \`step\`, \`target\`, \`mode\`, \`hints\` |
| \`science.label-diagram\` | Labeling parts of a diagram | \`image\`, \`labels[{id,text,target{x,y}}]\`, \`hints\` |
| \`science.image-label\` | Identifying regions on an image | \`image\`, \`regions[{id,title,x,y}]\`, \`hints\` |
