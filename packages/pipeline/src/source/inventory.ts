  ...
  for (const page of pages) {
    ...
    for (const segment of segments) {
      ...
      if (NIOS_EXERCISE_MARKER.test(trimmed)) {
        exerciseMode = true;
        units.push({ id, type: 'exercise', text: trimmed, location, extractionConfidence: 0.9, requiredCoverage: true });
        continue;
      }

      if (NIOS_OBJECTIVE_MARKER.test(trimmed)) {
        units.push({ id, type: 'objective', text: trimmed, location, extractionConfidence: 0.95, requiredCoverage: true });
        continue;
      }

      if (NIOS_REVIEW_MARKER.test(trimmed)) {
        exerciseMode = false;
        units.push({ id, type: 'review', text: trimmed, location, extractionConfidence: 0.9, requiredCoverage: false });
        continue;
      }

      if (NIOS_TEST_MARKER.test(trimmed)) {
        exerciseMode = false;
        units.push({ id, type: 'assessment', text: trimmed, location, extractionConfidence: 0.9, requiredCoverage: true });
        continue;
      }

      if (NIOS_EXAMPLE_MARKER.test(trimmed)) {
        units.push({ id, type: 'worked_example', text: trimmed, location, extractionConfidence: 0.9, requiredCoverage: true });
        continue;
      }
...
export async function buildSourceInventory(
  ...
  if (unclassifiedUnits.length > 0) {
    ...
    try {
      const result = await router.generateStructuredRaw('source_inventory', prompt, InventoryLLMResponseSchema, { temperature: 0.1 });
      ...
      for (const classification of result.classifications) {
        ...
        if (unit) {
          ...
          unit.requiredCoverage = ['worked_example', 'exercise', 'assessment', 'objective'].includes(classification.type);
        }
      }
    } catch {
